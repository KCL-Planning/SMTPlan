...
public:
    ...
    virtual void setName(const string & s) {name = s;};
...
class control_symbol : public symbol, public expression
{
    private:
        int cnt;
    public:
        control_symbol(const string & s) : symbol(s), cnt(0) {};
        virtual ~control_symbol() {};
        virtual void write(ostream & o) const;
        virtual void visit(VisitController * v) const;
        virtual void display(int ind) const;
        virtual string display() const;
        void setNum(int c) {cnt = -(100 + c);};
...
public:
	binary_expression(expression * a1,expression * a2) :
		arg1(a1), arg2(a2)
	{};
	binary_expression(const binary_expression & bexp) : arg1(const_cast<expression*>(bexp.getLHS())), arg2(const_cast<expression*>(bexp.getRHS()))
	{}
...
{
private:
    const special_val var; // these are E_HASHT, E_DURATION_VAR, E_TOTAL_TIME. these expressions can't be changed. Control Parameter should be changed!!
...
class comparison : public goal, public binary_expression
{
private:
    comparison_op op;
 ...
 *---------------------------------------------------------------------------*/
 ...
 public:
    ...
    class_def(const class_symbol * c,func_decl_list * fd) : dllHandle(0), name(c), funcs(fd) {};
...
// Added Control Parameters field:
    ...
    operator_() : symtab(0), parameters(0), control_parameters(0), precondition(0), effects(0) {};
...
public:
    ...
    control_symbol_table* getControlParams()
    {
