...
public:
     ...
     * Copy constructor.
     */
    instantiatedOp(const instantiatedOp & other)
        : id(other.id), op(new VAL::operator_(*other.op)), env(new VAL::FastEnvironment(*other.env)),
        hasControlParameter(other.hasControlParameter), isFinite(other.isFinite), isAux(other.isAux)
    {

    }

	instantiatedOp(const VAL::operator_ * o,VAL::FastEnvironment * e) : id(0), op(o), env(e), hasControlParameter(false), isFinite(false), isAux(false) {};
    ...
    static void incrementNonStaticLiteralCount() {
        assert(staticFactsAndLiteralsHaveBeenGivenIDs);
	...
	bool getHasControlParameter() const {return hasControlParameter;};
	void setHasControlParameter(bool y) {hasControlParameter = y;};

    bool getIsFinite() const {return isFinite;};
    void setIsFinite(bool z) {isFinite = z;};

    bool getIsAux() const {return isAux;};
    void setIsAux(bool z) {isAux = z;};
